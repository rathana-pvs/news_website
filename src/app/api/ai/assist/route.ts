  ...
  try {
    ...
    if (action === 'scrape_direct') {
      ...
      try {
        ...
        if (result.scrapedImageUrl) {
          try {
            ...
            if (imageRes.ok) {
              ...
              ext = ext.split(';')[0].trim() // Clean up charset info
            ...
            console.error('Failed to download/create media for scraped image:', imgErr)
          }
        }
        ...
        return NextResponse.json({ error: `Failed to scrape webpage: ${err.message}` }, { status: 400 })
      }
    }
